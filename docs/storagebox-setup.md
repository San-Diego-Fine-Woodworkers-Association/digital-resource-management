...
| `pull` | **read-only** | on-prem puller | same `/resourcespace` (or a sub-path of it) |
...
ssh-keygen -t ed25519 -f ./storagebox_push -C "resourcespace-backup-push" -N ""

# On the on-prem box:
ssh-keygen -t ed25519 -f ~/.ssh/storagebox_pull -C "resourcespace-backup-pull" -N ""
```
...
# Pull key -> pull sub-account (run from the on-prem box):
...
```
Then from wherever you generated the pull key (or a scratch box), confirm the
RO sub-account can see it and can't write:
```bash
ssh -p23 -i ~/.ssh/storagebox_pull <pull-user>@<box-host> ls   # should list files
# a write attempt should fail — the sub-account's own RO flag enforces this,
# there's no forced-command trick to verify here like the old rrsync setup.
```

## 10. Point the on-prem puller here

Continue with [onprem-pull-setup.md](./onprem-pull-setup.md) — `SERVER` is now
the **pull** sub-account/host, not the ResourceSpace server.
